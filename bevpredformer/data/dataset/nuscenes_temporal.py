...
Adapted from https://github.com/valeoai/PointBeV
"""
...
from bevpredformer.data.dataset.nuscenes_common import CAMREF, NuScenesDataset
from bevpredformer.utils.geom import (
...
class TemporalNuScenesDataset(NuScenesDataset):
    ...
    def _init_out_keys(
        ...
        if keep_input_flow_map:
            keys_to_keep.append("flow_map")
            keys_to_keep.append("flow_map_aug")
    ...
    def _get_inputs_bevaug(
        ...
        if self.is_train:
            ...
            aug_matrix = get_random_ref_matrix(coeffs=self.coeffs.bev_aug)
            bev_aug = np.stack(
                [
                    aug_matrix
    ...
    def __getitem__(self, index):
        ...
        data_bev, instance_map = self._get_outputs_bev(
            bev_records_T, egoPout_to_global, bev_aug
        )
    ...
    def _get_flow_label(self, data_bev, instance_dict,
                        instance_map, ignore_index=255):
        """Get the flow label from the instance image."""
                
        seq_len = len(data_bev)
        h, w = data_bev[0]['instance'].shape[-2:]
        ...
        for token, instance_id in instance_map.items():
            # Get timesteps where the instance is present
            present_timesteps = [
                t for t in range(seq_len) if instance_id in data_bev[t]['instance']
            ]
            ...
            # First frame
            instance_mask = (data_bev[0]['instance'] == instance_id).squeeze(0)
            ...
            for i, timestep in enumerate(present_timesteps):
                if i == 0:
                    continue
                instance_mask = (data_bev[timestep]['instance'] == instance_id).squeeze(0)
                prev_instance_mask = (data_bev[timestep-1]['instance'] == instance_id).squeeze(0)
        ...
        flow_aug = ignore_index * torch.ones(seq_len, 2, h, w)
        
        for token, instance_id in instance_map.items():
            # Get timesteps where the instance is present
            present_timesteps = [
                t for t in range(seq_len) if instance_id in data_bev[t]['instance_aug']
            ]
            ...
            # First frame
            instance_mask = (data_bev[0]['instance_aug'] == instance_id).squeeze(0)
            flow_aug[0, 1, instance_mask] = grid[0, instance_mask].mean(dim=0, keepdim=True).round() - grid[0, instance_mask]
            flow_aug[0, 0, instance_mask] = grid[1, instance_mask].mean(dim=0, keepdim=True).round() - grid[1, instance_mask]

            for i, timestep in enumerate(present_timesteps):
                if i == 0:
                    continue
                instance_mask = (data_bev[timestep]['instance_aug'] == instance_id).squeeze(0)
                prev_instance_mask = (data_bev[timestep-1]['instance_aug'] == instance_id).squeeze(0)
                if instance_mask.sum() == 0 or prev_instance_mask.sum() == 0:
                    continue
                # Centripetal backward flow is defined as displacement vector from each foreground pixel at time t to the object center of the associated instance identity at time t−1
                flow_aug[timestep, 1, instance_mask] = grid[0, prev_instance_mask].mean(dim=0, keepdim=True).round() - grid[0, instance_mask]
                flow_aug[timestep, 0, instance_mask] = grid[1, prev_instance_mask].mean(dim=0, keepdim=True).round() - grid[1, instance_mask]

        return flow, flow_aug
