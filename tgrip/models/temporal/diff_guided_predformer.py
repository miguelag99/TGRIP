...
from tgrip.models.layers.gated_transformer import GatedTransformer
...
class DiffGuidedPredFormerTemporal(nn.Module):
    ...
    def __init__(
        ...
        """PredFormer Temporal implementation from PredFormer: Modeling Motion in Spacetime
        https://arxiv.org/abs/2410.04733
        ...
        # Diff guided motion module
        self.diff_motion_module = DiffMotionModule(
    ...
    def forward(self, x):
        ...
        output (torch.Tensor): [B, T, C, H, W] refined BEV feature maps
        """
        b, t, c, h, w = x.shape
        assert (
            t == self.num_past_frames
        ), f"Expected {self.num_past_frames} past frames, got {t}"

        # Extract difference guided motion features and add them to the bev feats
        diff_feats = self.diff_motion_module(x)
        x = x + diff_feats.expand(-1, self.num_past_frames, -1, -1, -1)
        ...
        patches = [
            p.flatten(2).transpose(1, 2) for p in patches
        ]  # [B, num_patches, embed_dim]
        tokens = torch.stack(patches, dim=1)  # [B, T, num_patches, embed_dim]

        # Future query tokens
        if self.query_type == "learnable":
            queries = self.future_query.expand(
                b, -1, -1, -1
            )  # [B, num_past_frames, num_patches, embed_dim]
        elif self.query_type == "linear":
            queries = self.query_linear(rearrange(tokens, "b tp np c -> b np (tp c)"))
            queries = rearrange(
        ...
        ph, pw = (
            self.bev_size[0] // self.patch_size,
            self.bev_size[1] // self.patch_size,
        )
        queries = rearrange(queries, "b tp c (ph pw) -> b (tp c) ph pw", ph=ph, pw=pw)
        queries = self.upsample(queries)
        queries = rearrange(
            queries, "b (tp c) h w -> b tp c h w", tp=self.num_past_frames
        )

        return queries

## Difference guided motion module 
# https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=10932678

class DiffMotionModule(nn.Module):
    def __init__(
        self,
        in_channels: int = 256,
        out_channels: int = 256,
        ...
        self.in_conv = nn.Sequential(
            nn.Conv2d(
                in_channels=in_channels*(num_past_frames-1),
                out_channels=1,
                kernel_size=3,
                stride=2,
                padding=1,
        ...
        self.path1 = nn.Sequential(
            ...
            nn.AdaptiveMaxPool2d(1),  # Global max pooling (outputs 1x1)
        )
        
        self.path2 = nn.Conv2d(
            in_channels=1,
            out_channels=1,
            kernel_size=1,
            stride=1,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: motion features tensor with shape [B, T, 1, H, W]
        """
        b, t, c, h, w = x.shape
        
        # Concatenate initial differential features
        diff_feats = []
        for i in range(t-1):
            x_mot = x[:, i+1] - x[:, i]
            diff_feats.append(x_mot)

        diff_feats = torch.concat(diff_feats, dim=1)  # [B, C*(T-1), H, W]
        
        dt = self.in_conv(diff_feats)  # [B, C, H/2, W/2]
        w1 = self.path1(dt)  # [B, 1, 1, 1]
        w1 = w1.expand(-1, -1, h//2, w//2)  # [B, 1, H/2, W/2]
        
        path12 = self.softmax(rearrange(self.path2(dt).mT * w1, 'b 1 h w -> b 1 (h w)'))
        path12 = rearrange(path12, 'b 1 (h w) -> b 1 h w', h=h//2, w=w//2)
        
        g = self.path3(dt)  # [B, 1, 1, 1]
        g = g.expand(-1, -1, h//2, w//2)  # [B, 1, H/2, W/2]
        
        non = path12 * g  # [B, 1, H/2, W/2]
        
        motion_features = self.upsample(self.w(non) + dt)
        
        # Expand to include time dimension
        motion_features = motion_features.unsqueeze(1).expand(b, t, 1, h, w)
