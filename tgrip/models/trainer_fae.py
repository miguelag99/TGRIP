...
class FaEPredictionTrainer(LightningModule):
    ...
    def configure_optimizers(self):
        ...
        scheduler_conf = OmegaConf.to_container(self.hparams.scheduler, resolve=True)
        
        interval = scheduler_conf.pop("interval", "epoch") # Default to 'epoch' if not present

        if "OneCycleLR" in scheduler_conf.get("classname", ""):
            # Change target for Hydra
            scheduler_conf["_target_"] = scheduler_conf.pop("classname")
            
            # Lightning's official estimator
            # Automatically calculates: (batches / gpus / accumulation) * epochs
            scheduler_conf["total_steps"] = self.trainer.estimated_stepping_batches
            
            # Synchronize the maximum LR
            scheduler_conf["max_lr"] = optimizer.param_groups[0]["lr"]
            
            # Force interval to 'step' for OneCycleLR
            interval = "step"

        # Instantiate with Hydra, passing the processed dict
